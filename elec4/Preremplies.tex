...
\element{securite}{
  \begin{questionmult}{sec2}
    ...
    \begin{reponsesd}
      \input{"|./genexo.py --seed \ySeed~ --mode S1 --level 0 --qref toto --all"}
    \end{reponsesd}
  \end{questionmult}
}
...
\element{securite}{
  \begin{questionmult}{sec3}
    ...
    \FPeval\ySeed{trunc(random*1000,0) }
    \begin{reponsesd}
      \input{"|./genexo.py --seed \ySeed~ --mode S2 --level 0 --qref toto --all"}
      \bonne{Démonter un micro-onde débranché}\bareme{m=0}
   \end{reponsesd}
   \explain{Certains appareils électriques, comme les micro-ondes commportent des condensateurs haute-tension qui peuvent rester chragés après débranchement de l'appareil.}
  \end{questionmult}
}
...
\restituegroupe{mesures2}
\restituegroupe{mesures3}
%\end{multicols}
...
\restituegroupe{introcalculs1}
\restituegroupe{introcalculs2}
%\restituegroupe{introcalculs}

\section{Calculs d'intensités du courant électrique}
\begin{multicols}{2}
\restituegroupe{calculs1}
\restituegroupe{calculs2}
\restituegroupe{calculs3}
\end{multicols}
